...
int HashTable::SendAllEntriesByName( char _name[32] ,DataUser * _data ){
    int index = hash( _name ); //Calcula en que indice debería estar
    return headsArray[ index ].SendAllEntries( _name ,_data);   //Imprime los existentes en la lista correspondiente.
}
